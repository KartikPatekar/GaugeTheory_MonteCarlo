...
double randAmp;
inline vector< double > FlipHiggsField(vector < double > H, double dA=0.1*Hvev)
{
	Hret[0]=H[0]+1.0*dA*(1.0-2.0*std_rand());
	Hret[1]=H[1]+1.0*dA*(1.0-2.0*std_rand());
	Hret[2]=H[2]+1.0*dA*(1.0-2.0*std_rand());
	...
	for(x=0; x<L;x++)
	{
		...
		for(y=0; y<L;y++)
		{
			...
			for(t=0; t<L;t++)
			{
				...
				H[1][x][y][t]=FlipHiggsField(H[1][x][y][t], 1.15*Hvev);
				H[2][x][y][t]=FlipHiggsField(H[2][x][y][t], 1.15*Hvev);
				H[3][x][y][t]=FlipHiggsField(H[3][x][y][t], 1.15*Hvev);
				H[4][x][y][t]=FlipHiggsField(H[4][x][y][t], 1.15*Hvev);
			}
		}
	}
}
...
void HiggsUpdate(int xcord, int ycord, int tcord)
{
	...
	for(int l=1; l<=4; l++)
	{
		Hnew=FlipHiggsField(H[l][xcord][ycord][tcord]);
