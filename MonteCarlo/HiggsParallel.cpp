...
double calcAction(int xcord, int ycord, int tcord, vector<double> Hfield, int Hnum)
{
	...
	vector<  double  > H_old{ 0.0, 0.0, 0.0 };				//For saving old H field.
	r={xcord, ycord, tcord};
...
//MC sweem all sites
void MCsweepSequential(int starty, int startt)
{	
	int x,y,t;
	for(x=0; x<L; x++)
	{
		for(y=starty; y<L/2+starty; y++)
		{
			for(t=startt; t<L/2+startt; t++)
			{
				HiggsUpdate(x,y,t);							
			}		
		}
...
void ParallelProg()
{
	thread th00(MCsweepSequential,0,0);
	thread th01(MCsweepSequential,0,L/2);
	thread th10(MCsweepSequential,L/2,0);
	thread th11(MCsweepSequential,L/2,L/2);
